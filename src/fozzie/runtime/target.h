...
/*
 * FOZZIE_OPTIONAL declares a symbol the final binary may leave undefined; the
 * runtime compares its address with null before calling it. Mach-O only allows
 * that for symbols the link names with -U, which the runtime's Buck target
 * exports for every symbol declared this way.
 */
