...
/* Not every libc offers a way to skip SIGPIPE per call; see open_cloexec_socket. */
...
FOZZIE_NOCOV static int open_cloexec_socket(void) {
    ...
    /* A controller that vanishes must end the target through a failed send,
     * never through SIGPIPE, on platforms where MSG_NOSIGNAL is not enough. */
    if (fd >= 0) {
        int one = 1;
...
FOZZIE_NOCOV static int connect_control_socket(const char* path) {
    ...
    state.socket_fd = open_cloexec_socket();
...
#if defined(__APPLE__)
/*
 * Darwin has no PR_SET_PDEATHSIG. A thread parks on a process filter for the
 * controller and delivers the same SIGKILL when it exits, so a target stuck
 * inside the harness cannot outlive its campaign. Descendants forked by the
 * harness inherit neither the thread nor the kqueue, as on Linux.
 */
FOZZIE_NOCOV static void* watch_parent(void* argument) {
    (void)argument;
    ...
    if (parent > 1) {
        ...
        EV_SET(&change, (uintptr_t)parent, EVFILT_PROC, EV_ADD, NOTE_EXIT, 0, NULL);
        if (kevent(queue, &change, 1, NULL, 0, NULL) != 0) {
            if (errno != ESRCH) {
                return NULL;
            }
            /* The controller exited between getppid and the registration. */
        } else {
            struct kevent event;
            int received;
            do {
                received = kevent(queue, NULL, 0, &event, 1, NULL);
...
FOZZIE_NOCOV static void start_parent_watch(void) {
    ...
    (void)pthread_create(&thread, &attributes, watch_parent, NULL);
