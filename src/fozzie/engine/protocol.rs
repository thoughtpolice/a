...
/// The target's side of the protocol: encoding what the runtime sends,
/// decoding what the engine sends, and the blocking stream helpers. The
/// engine only ever decodes Hello and Done out of its own buffers and
/// encodes Run and Stop, so the rest exists for tests that play the target.
#[cfg(test)]
mod peer {
    use super::*;
    use std::io::Read;
    ...
    fn read_frame<const SIZE: usize>(
...
#[cfg(test)]
mod tests {
    use super::peer::{CAP_PC_TABLE, CAP_TRACE_CMP};
    use super::*;
    use std::io::Cursor;
