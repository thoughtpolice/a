...
pub struct ExecutorConfig {
    ...
    /// Named files the target is told about, by absolute path.
    pub resources: Vec<(String, PathBuf)>,
    pub max_input: usize,
    pub timeout: Duration,
    pub feature_capacity: usize,
    pub cmp_capacity: usize,
    /// Instrumentation profile of the target, `none` or `address`.
...
impl ExecutorConfig {
    pub fn from_options(options: &TargetOptions) -> Result<Self> {
        ...
        let resources = options
            ...
            .map(|(name, path)| {
                fs::canonicalize(path)
                    ...
                    .with_context(|| format!("resolving resource {name} at {}", path.display()))
            })
        ...
        Ok(Self {
            ...
            sanitizer: options.sanitizer.clone(),
        })
    }
}
...
pub struct Observation {
    ...
    /// Empty when the executor was told not to collect comparisons.
    pub comparisons: Vec<CmpObservation>,
    pub features_truncated: bool,
    /// The target ran out of comparison capacity while the executor was
    /// collecting comparisons. Truncation of feedback nobody reads is not
    /// reported.
...
impl PersistentExecutor {
    ...
    /// Chooses whether later runs decode the target's comparison region.
    /// The target records comparisons either way; decoding tens of thousands
    /// of entries per run is the engine's dominant cost for targets that
    /// compare a lot, and it buys nothing once the dictionary is full.
    pub fn set_collect_comparisons(&mut self, collect: bool) {
        self.collect_comparisons = collect;
    }
    ...
    /// The counter count the live target declared in its Hello frame, or
    /// `None` between sessions. It is a property of the binary, so any
    /// session's value stands for the campaign.
    pub fn counter_count(&self) -> Option<u64> {
        self.session.as_ref().map(|session| session.counter_count)
    }
    ...
    pub fn run(&mut self, input: &[u8]) -> Result<Execution> {
        ...
        if done.status == DONE_HARNESS_NONZERO {
            ...
            let (_, stderr) = session.terminate();
        ...
        let comparisons = if self.collect_comparisons {
            session.shm.read_cmp(done.cmp_count)?
        } else {
            Vec::new()
        };
        Ok(Execution::Ok(Observation {
            ...
            comparisons_truncated: self.collect_comparisons
                && done.done_flags & DONE_COMPARISONS_TRUNCATED != 0,
        }))
    }
    ...
    fn failed_run(&mut self, timed_out: bool, error: Error) -> Result<Execution> {
        ...
        if timed_out {
            ...
            // particular can hold a crashing process for a while before it
            // becomes reapable while its crash reporter is notified.
            if let Some(status) = status.filter(|status| terminated_on_its_own(*status)) {
                return Ok(classify_exit(status, stderr));
            }
            return Ok(Execution::Finding(Finding::new(
                FindingKind::Hang,
        ...
        match outcome {
            ...
            Ok(DisconnectOutcome::Forced(status)) if terminated_on_its_own(status) => {
...
impl StderrCapture {
    fn spawn(stderr: ChildStderr) -> Result<Self> {
        ...
        let nonblocking = flags | libc::O_NONBLOCK;
        if flags < 0 || unsafe { libc::fcntl(descriptor, libc::F_SETFL, nonblocking) } < 0 {
...
impl Session {
    fn spawn(config: &ExecutorConfig, worker_root: &Path) -> Result<Self> {
        ...
        // Unix socket pathnames are limited to about a hundred bytes
        // independently of filesystem paths. Keep this private directory
        // short even when the campaign directory or TMPDIR is deeply nested.
        ...
        command
            .args(config.resources.iter().map(|(name, path)| {
                let mut argument = std::ffi::OsString::from(format!("--fozzie-resource={name}="));
                argument.push(path);
                argument
            }))
            ...
            .stderr(Stdio::piped())
            .process_group(0);
        if cfg!(target_os = "macos") && config.sanitizer == "address" {
            // Darwin's nano malloc zone cannot reserve its range next to the
            // ASan shadow and complains on stderr; LLVM's own ASan tests turn
            // it off the same way.
            command.env("MallocNanoZone", "0");
        }
        request_parent_death_signal(&mut command);
        let child = command
            .spawn()
            .with_context(|| format!("starting target {}", config.target.display()))?;
        let mut child = ChildGuard::new(child);
        let child_handle =
            open_process_handle(child.get_mut()).context("opening target process handle")?;
    ...
    fn wait_for_socket(
        ...
        loop {
            ...
            let mut descriptors = [
                ...
                libc::pollfd {
                    fd: self.child_handle.as_raw_fd(),
                    events: libc::POLLIN,
                    revents: 0,
                },
            ];
    ...
    /// Kills the target and returns the status its reap produced, if any,
    /// with the stderr snapshot.
    fn terminate(&mut self) -> (Option<ExitStatus>, Vec<u8>) {
        let status = self
            .child
            .take()
            .and_then(|mut child| kill_process_group(&mut child));
        self.stderr.finish();
        (status, self.read_stderr())
    }
...
fn child_pid(child: &Child) -> std::io::Result<libc::pid_t> {
    libc::pid_t::try_from(child.id())
        .map_err(|_| std::io::Error::new(ErrorKind::InvalidInput, "target PID does not fit pid_t"))
}

/// Opens a descriptor that polls readable once the direct child has exited.
#[cfg(target_os = "linux")]
fn open_process_handle(child: &Child) -> std::io::Result<OwnedFd> {
...
/// Opens a descriptor that polls readable once the direct child has exited.
///
/// A kqueue holding a process filter stands in for the pidfd: Darwin
/// implements poll(2) over kqueue and lets one kqueue watch another, and the
/// exit event is never drained, so the descriptor stays readable afterwards
/// just as a pidfd does.
#[cfg(target_os = "macos")]
fn open_process_handle(child: &Child) -> std::io::Result<OwnedFd> {
    let pid = child_pid(child)?;
    ...
    // SAFETY: kqueue returned a fresh, owned descriptor above.
    let handle = unsafe { OwnedFd::from_raw_fd(queue) };
    // SAFETY: fcntl only sets the descriptor flag of this owned descriptor.
    if unsafe { libc::fcntl(handle.as_raw_fd(), libc::F_SETFD, libc::FD_CLOEXEC) } < 0 {
        return Err(std::io::Error::last_os_error());
    }
    let exit = kevent_change(pid as libc::uintptr_t, libc::EVFILT_PROC, libc::NOTE_EXIT);
    match register_kevent(&handle, &exit) {
        Ok(()) => Ok(handle),
        // The child has already exited, and the kernel attaches no filter to
        // a process it considers gone. Queue an already-triggered user event
        // instead so the handle reads ready at once, as a pidfd of an exited
        // process does.
        Err(error) if error.raw_os_error() == Some(libc::ESRCH) => {
            let exited = kevent_change(0, libc::EVFILT_USER, libc::NOTE_TRIGGER);
            register_kevent(&handle, &exited)?;
            Ok(handle)
        }
        Err(error) => Err(error),
    }
...
#[cfg(target_os = "macos")]
fn register_kevent(queue: &OwnedFd, change: &libc::kevent) -> std::io::Result<()> {
    // SAFETY: change points to one initialized kevent record for the duration
    // of the call, and no event list is supplied to be written.
    let result = unsafe {
        libc::kevent(
            queue.as_raw_fd(),
            change,
...
/// Asks the kernel to kill the target should the controller die first.
#[cfg(target_os = "linux")]
...
/// Darwin has no such request; the target runtime watches its parent from a
/// thread instead.
#[cfg(not(target_os = "linux"))]
fn request_parent_death_signal(_command: &mut Command) {}

/// Kills the target's process group and reaps the direct child, returning
/// its status when the reap succeeds.
fn kill_process_group(child: &mut Child) -> Option<ExitStatus> {
...
/// Whether a status reaped after Fozzie sent SIGKILL records the target's own
/// termination rather than that kill.
fn terminated_on_its_own(status: ExitStatus) -> bool {
...
mod tests {
    ...
    #[test]
    fn only_a_kill_status_is_attributed_to_fozzie() {
        assert!(!terminated_on_its_own(ExitStatus::from_raw(libc::SIGKILL)));
        assert!(terminated_on_its_own(ExitStatus::from_raw(libc::SIGABRT)));
        assert!(terminated_on_its_own(ExitStatus::from_raw(70 << 8)));
    }
}
