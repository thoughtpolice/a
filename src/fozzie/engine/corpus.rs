...
/// Atomic replacement without fsync. Readers still never see a partial
/// file because the rename is what publishes it; only durability across a
/// power loss is given up, the right trade for state that is rewritten
/// every second and stale after a crash anyway.
pub fn persist_replace_unsynced(path: &Path, contents: &[u8]) -> Result<()> {
    replace(path, contents, false)
}

fn replace(path: &Path, contents: &[u8], durable: bool) -> Result<()> {
    let parent = publication_parent(path)?;
    let mut temporary = tempfile::NamedTempFile::new_in(parent)
        .with_context(|| format!("creating temporary file in {}", parent.display()))?;
    temporary
    ...
    if durable { sync_parent(parent) } else { Ok(()) }
}
...
mod tests {
    ...
    #[test]
    fn unsynced_replacement_overwrites_and_leaves_no_staging_file() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("status");
        persist_replace_unsynced(&path, b"one").unwrap();
        persist_replace_unsynced(&path, b"two").unwrap();
