...
pub struct RecordedFinding {
    ...
    pub detail: String,
    pub repro: String,
    /// A `minimize` invocation over the recorded input.
    pub minimize: String,
}
...
struct ReplayManifest {
    ...
    #[serde(default)]
    resources_base64: Vec<(String, String)>,
    input_base64: String,
    timeout_ms: u64,
    max_input: usize,
    feature_capacity: usize,
...
impl ArtifactSink {
    ...
    pub fn record(
        ...
        let replay = ReplayManifest {
            ...
            target_args_base64: self
                ...
                .collect(),
            resources_base64: self
                .resources
                .iter()
                .map(|(name, path)| (name.clone(), STANDARD.encode(path.as_os_str().as_bytes())))
                .collect(),
        ...
        let minimize = self.minimize_command(&input_path);
    ...
    /// The options every invocation of the target shares: the executable,
    /// its arguments and resources, and the executor limits.
    fn target_options(&self) -> String {
        let mut options = format!("--target {}", shell_word(&self.target));
        for argument in &self.target_args {
            options.push_str(" --target-arg ");
            options.push_str(&shell_quote(&argument.to_string_lossy()));
        }
        for (name, path) in &self.resources {
            options.push_str(" --resource ");
            options.push_str(&shell_quote(&format!("{name}={}", path.to_string_lossy())));
        }
        options.push_str(&format!(
            " --timeout-ms {} --max-input {} --feature-capacity {} --cmp-capacity {}",
            self.timeout_ms, self.max_input, self.feature_capacity, self.cmp_capacity,
        ));
        options
    }

    fn minimize_command(&self, input_path: &Path) -> String {
        format!(
            "buck2 run root//src/fozzie/engine:fozzie -- minimize {} --input {}",
            self.target_options(),
            shell_word(input_path)
        )
    }
    ...
    fn reproduction_command(&self, encoded: &str) -> Option<String> {
        ...
        for (name, path) in &self.resources {
            command.push_str(" --resource ");
            command.push_str(&shell_quote(&format!("{name}={}", path.to_str()?)));
        }
...
mod tests {
    ...
    fn records_input_and_machine_readable_metadata() {
        ...
        let config = ExecutorConfig {
            ...
            resources: vec![("oracle".to_owned(), directory.path().join("oracle.txt"))],
            max_input: 4096,
        ...
        assert_eq!(replay.target.resources, config.resources);
        assert!(
            recorded.repro.contains("--resource 'oracle="),
            "{}",
            recorded.repro
        );
        assert!(
            recorded
                .minimize
                .starts_with("buck2 run root//src/fozzie/engine:fozzie -- minimize --target ")
                && recorded.minimize.contains("--resource 'oracle=")
                && recorded
                    .minimize
                    .ends_with(&format!(" --input {}", shell_word(&recorded.input_path))),
            "{}",
            recorded.minimize
        );
        assert_eq!(metadata["minimize"], recorded.minimize);
