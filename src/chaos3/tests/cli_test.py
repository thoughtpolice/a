...
def server(*args, stderr_log=None):
    with subprocess.Popen(
        ...
        try:
            ...
            prefix = "chaos3 listening on http://"
...
def signed_request(
    address, method, path, body=b"", headers=None, access_key="chaos3", secret="chaos3"
):
...
def missing_object_statuses(address, count):
    """Request a missing object repeatedly and return the status of each answer.

    Every answer must be the ordinary NoSuchKey error or an injected SlowDown.
    """
    statuses = []
    for _ in range(count):
        status, _, body = request(address, "GET", "/celld/missing")
        expected = {404: "NoSuchKey", 503: "SlowDown"}.get(status)
        if xml_values(body, "Code") != [expected]:
            raise AssertionError(f"unexpected answer {status}: {body!r}")
        statuses.append(status)
    return statuses


class CliTests(unittest.TestCase):
    def test_chaos_replays_a_bounded_campaign_across_processes(self):
        runs = []
        for _ in range(2):
            logs = []
            with server(
                "--fault-seed=42", "--chaos=storage-v1", "--chaos-trace",
                "--chaos-warmup-requests=4", "--chaos-requests=32",
                stderr_log=logs,
            ) as address:
                statuses = missing_object_statuses(address, 40)
            ...
            traces = [line for line in logs if line.startswith("chaos3 chaos trace:")]
            self.assertEqual(len(traces), 44, logs)
    ...
    def test_occupied_port_fails_without_announcing_readiness(self):
        with socket.socket() as listener:
            ...
            self.assertNotIn("chaos3 listening", result.stdout)
    ...
    def test_truncated_body_is_a_failed_read_on_a_real_socket(self):
        with server("--failpoint=s3.get_object.body=1*off->1*return") as address:
            payload = bytes(range(256)) * 512
            self.assertEqual(request(address, "PUT", "/celld/chunked", payload)[0], 200)
            connection = http.client.HTTPConnection(address, timeout=5)
            try:
                connection.request("GET", "/celld/chunked")
                response = connection.getresponse()
                ...
                with self.assertRaises(http.client.IncompleteRead) as failure:
                    response.read()
                self.assertEqual(failure.exception.partial, payload[: 4 * 1024])
            finally:
                connection.close()
            self.assertEqual(request(address, "GET", "/celld/chunked")[2], payload)
    ...
    def test_wrong_signed_credentials_are_rejected_without_writing(self):
        with server() as address:
            for access_key, secret, code in [
                ("wrong", "chaos3", "NotSignedUp"),
                ("chaos3", "wrong", "SignatureDoesNotMatch"),
            ]:
