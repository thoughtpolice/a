...
fn counts(chaos: &Chaos) -> (u64, u64, u64, u64, u64, u64) {
...
async fn warmup_and_recovery_count_requests_including_failed_ones() {
    ...
    for index in 1..=4 {
        ...
        assert_eq!(counts(&chaos), (0, 0, 0, 0, 0, 0));
    }
...
async fn retrying_get(service: &S3Service, key: &str) -> String {
    for _ in 0..32 {
        ...
        match response.status() {
            StatusCode::OK => {
                // A truncated body is a failed read, retried like an error.
                if let Ok(collected) = BodyExt::collect(response.into_body()).await {
                    return String::from_utf8(collected.to_bytes().to_vec()).expect("UTF-8 object");
                }
            }
...
async fn immutable_object_client_reconciles_ambiguous_commits_then_audits_recovery() {
    ...
    assert!(chaos.snapshot().truncations > 0);
    ...
    let recovered = counts(&chaos);
    let keys = xml_values(&get_text(&service, "/celld?list-type=2").await, "Key");
    assert_eq!(
        keys,
        expected
            .iter()
    ...
    assert!(xml_values(&get_text(&service, "/celld?list-type=2").await, "Key").is_empty());
