...
def _uv_python_binary_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    lint_dir = ctx.attrs.lint_dir if ctx.attrs.lint_dir else "src/tools/bizarro"
    lint_commands = []
...
_uv_python_binary = rule(
    ...
    attrs = {
        ...
        "lint_dir": attrs.string(default = "", doc = "Directory to lint (defaults to src/tools/bizarro)"),
...
def _uv_tool_format_impl(ctx: AnalysisContext) -> list[Provider]:
    ...
    format_dir = ctx.attrs.format_dir if ctx.attrs.format_dir else "src/tools/bizarro"
...
_uv_tool_format = rule(
    ...
    attrs = {
        "format_dir": attrs.string(default = "", doc = "Directory to format"),
    }
)
