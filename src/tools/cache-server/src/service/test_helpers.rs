...
/// A handle that spawns on whatever runtime the caller is already running,
/// with no telemetry behind it. The services under test only ever `spawn`
/// through it, so an inert handle exercises the same paths.
