...
Usage:
  scratch_image.py build \\
      ...
      [--cmd CMD] [--env KEY=VAL]... [--label KEY=VAL]... [--user USER] \\
      [--port N/tcp]... [--arch amd64] [--os linux] \\
      [--base-layer-count N] [--policy FILE]

  scratch_image.py validate LAYOUT [--expected-cmd CMD] \\
      [--base-layer-count N] [--policy FILE]

`build` writes an OCI image layout directory (with `oci-layout`,
`index.json`, and a `blobs/sha256/` tree), tagged `latest`. `validate`
re-reads a finished layout through the same checks: descriptor digests,
diff IDs, every layer's metadata, and the composition policy (see
../policy.txt for the file format) applied to the layers after the
first --base-layer-count trusted ones.
"""
...
from common import BoundedReader, UnsafeInputError, canonical_name, copy_exact, link_parts
...
def load_policy(path: Path) -> CompositionPolicy:
    """Read a policy file: `composable PREFIX` and `sealed PREFIX` lines."""
    ...
    for number, raw in enumerate(path.read_text().splitlines(), start=1):
        ...
        if not line or line.startswith("#"):
            continue
        kind, _, prefix = line.partition(" ")
        prefix = canonical_name(prefix.strip(), what=f"{path}:{number}")
        if kind == "composable":
            composable.add(prefix)
        elif kind == "sealed":
            sealed.add(prefix)
        else:
            raise UnsafeInputError(
                f"{path}:{number}: expected 'composable PREFIX' or 'sealed PREFIX'"
            )
    if not composable:
...
def canonical_member_name(name: str) -> str:
    canonical = canonical_name(name, what="layer member")
    if any(part.startswith(".wh.") for part in canonical.split("/")):
        raise UnsafeInputError(f"OCI whiteouts are forbidden in minimos layers: {name!r}")
    return canonical


def validate_layer(path: Path) -> tuple[dict[str, LayerEntry], str]:
    """Check every extraction-relevant field of a layer tar in one pass.

    Returns the ordered member metadata and the layer's diff ID, the
    sha256 of the uncompressed stream, which the same pass computes.
    """
    ...
    with ExitStack() as stack:
        try:
            ...
            bounded = BoundedReader(stream, MAX_LAYER_STREAM_SIZE, digest=digest)
            ...
            raise UnsafeInputError(f"layer is not a readable tar archive: {path}") from error
        for count, member in enumerate(archive, start=1):
            if count > MAX_LAYER_ENTRIES:
                raise UnsafeInputError(f"layer has more than {MAX_LAYER_ENTRIES} entries")
            name = canonical_member_name(member.name)
            if name in entries:
                raise UnsafeInputError(f"duplicate layer destination: {name}")
            if member.size < 0 or member.size > MAX_LAYER_FILE_SIZE:
                raise UnsafeInputError(f"layer member has invalid size: {name}")
            total_size += member.size
            if total_size > MAX_LAYER_CONTENT_SIZE:
                raise UnsafeInputError("layer expanded content exceeds size limit")

            mode = member.mode
            if mode < 0 or mode > 0o7777:
                raise UnsafeInputError(f"layer entry has an invalid mode: {name}")
            if member.uid < 0 or member.uid > 2**31 - 1:
                raise UnsafeInputError(f"layer entry has an invalid uid: {name}")
            if member.gid < 0 or member.gid > 2**31 - 1:
                raise UnsafeInputError(f"layer entry has an invalid gid: {name}")
            if mode & (stat.S_ISUID | stat.S_ISGID):
                raise UnsafeInputError(f"setuid/setgid layer entry: {name}")
            if member.isdir():
                ...
                if mode & stat.S_IWOTH:
                    raise UnsafeInputError(f"world-writable layer file: {name}")
                kind = "regular"
            elif member.issym():
                link_parts(name.split("/")[:-1], member.linkname, what=f"symlink {name!r}")
                kind = "symlink"
            else:
                raise UnsafeInputError(
            ...
            if getattr(member, "sparse", None):
                raise UnsafeInputError(f"sparse layer entries are forbidden: {name}")
        ...
        # The parser stops at the end-of-archive marker; the diff ID covers
        # the padding after it too.
        while bounded.read(1 << 16):
            pass
    return entries, "sha256:" + digest.hexdigest()
...
def _descriptor_blob(layout: Path, descriptor: dict, *, what: str,
    ...
    if not isinstance(digest, str) or not re.fullmatch(r"sha256:[0-9a-f]{64}", digest):
        raise UnsafeInputError(f"{what} has an invalid digest")
    if not isinstance(size, int) or isinstance(size, bool) or size < 0:
        raise UnsafeInputError(f"{what} has an invalid size")
    if max_size is not None and size > max_size:
        raise UnsafeInputError(f"{what} exceeds {max_size} bytes")
    blob = _blob_path(layout, digest)
    try:
        metadata = blob.lstat()
    except FileNotFoundError as error:
        raise UnsafeInputError(f"{what} blob is missing: {digest}") from error
...
def _read_json(path: Path, *, what: str) -> dict:
    if path.stat().st_size > MAX_JSON_BLOB_SIZE:
        raise UnsafeInputError(f"{what} JSON exceeds {MAX_JSON_BLOB_SIZE} bytes")
    try:
        value = json.loads(path.read_bytes())
    except (json.JSONDecodeError, OSError) as error:
        raise UnsafeInputError(f"{what} is not valid JSON") from error
    if not isinstance(value, dict):
        raise UnsafeInputError(f"{what} JSON is not an object")
    return value
...
def validate_oci_layout(layout: Path, *, base_layer_count: int = 0,
    ...
    if (
        ...
        raise UnsafeInputError(f"OCI layout is not a directory: {layout}")
    layout = layout.resolve()
    oci_layout_path = layout / "oci-layout"
    if not oci_layout_path.is_file() or oci_layout_path.is_symlink():
        raise UnsafeInputError("OCI oci-layout is missing or unsafe")
    oci_layout = _read_json(oci_layout_path, what="OCI layout metadata")
    if oci_layout != {"imageLayoutVersion": "1.0.0"}:
        raise UnsafeInputError("OCI layout metadata has an unsupported version")

    index_path = layout / "index.json"
    if not index_path.is_file() or index_path.is_symlink():
        raise UnsafeInputError("OCI index.json is missing or unsafe")
    index = _read_json(index_path, what="OCI index")
    if index.get("schemaVersion") != 2:
        raise UnsafeInputError("OCI index has an unsupported schema version")
    if index.get("mediaType") != "application/vnd.oci.image.index.v1+json":
        raise UnsafeInputError("OCI index has an unsupported media type")
    manifests = index.get("manifests")
    if not isinstance(manifests, list) or len(manifests) != 1:
        raise UnsafeInputError("OCI index must reference exactly one manifest")
    manifest_descriptor = manifests[0]
    if not isinstance(manifest_descriptor, dict):
        raise UnsafeInputError("OCI manifest descriptor is not an object")
    if manifest_descriptor.get("mediaType") != "application/vnd.oci.image.manifest.v1+json":
        raise UnsafeInputError("OCI index references an unsupported manifest type")
    ...
    if manifest.get("schemaVersion") != 2:
        raise UnsafeInputError("OCI manifest has an unsupported schema version")
    if manifest.get("mediaType") != "application/vnd.oci.image.manifest.v1+json":
        raise UnsafeInputError("OCI manifest has an unsupported media type")

    config_descriptor = manifest.get("config")
    if not isinstance(config_descriptor, dict):
        raise UnsafeInputError("OCI manifest config descriptor is invalid")
    if config_descriptor.get("mediaType") != "application/vnd.oci.image.config.v1+json":
        raise UnsafeInputError("OCI manifest references an unsupported config type")
    ...
    if not isinstance(rootfs, dict) or rootfs.get("type") != "layers":
        raise UnsafeInputError("OCI config has an invalid rootfs")
    ...
    for index, descriptor in enumerate(layers):
        if not isinstance(descriptor, dict):
            raise UnsafeInputError(f"OCI layer {index} descriptor is invalid")
        media_type = descriptor.get("mediaType")
        if media_type not in {
            "application/vnd.oci.image.layer.v1.tar",
            "application/vnd.oci.image.layer.v1.tar+gzip",
        }:
            raise UnsafeInputError(f"OCI layer {index} has an unsupported media type")
        ...
        entries, diff_id = validate_layer(blob)
        apply_layer_state(
            entries,
            effective_state,
...
def ingest_layer(source: Path, blobs: Path, index: int) -> tuple[Path, str]:
    """Copy a layer into the blob store, hashing it on the way.

    Everything after this reads the private copy, so a pathname swapped
    under a direct CLI run cannot change the bytes the image is built
    from, while Buck's declared-input symlinks still work.
    """
    try:
        fd = os.open(source, os.O_RDONLY | os.O_CLOEXEC)
    except OSError as error:
        raise UnsafeInputError(f"layer input is missing: {source}") from error
    digest = hashlib.sha256()
    incoming = blobs / f".incoming-{index}"
    with os.fdopen(fd, "rb") as stream, open(incoming, "xb") as blob:
        metadata = os.fstat(stream.fileno())
        if (
            not stat.S_ISREG(metadata.st_mode)
            or metadata.st_size < 0
            or metadata.st_size > MAX_LAYER_BLOB_SIZE
        ):
            raise UnsafeInputError(f"layer input is not a bounded regular file: {source}")
        copy_exact(stream, blob, metadata.st_size, what=f"layer input {source}",
                   digest=digest)
    blob_path = blobs / digest.hexdigest()
    os.replace(incoming, blob_path)
    return blob_path, "sha256:" + digest.hexdigest()


def write_blob(data: bytes, output: Path, digest: str) -> None:
    path = _blob_path(output, digest)
...
def _validate(args, policy: CompositionPolicy) -> int:
    try:
    ...
    except (UnsafeInputError, OSError) as error:
    ...
    if not isinstance(command, list) or not command or not all(
        ...
        return 1
    if args.expected_cmd is not None:
        expected = [item for item in args.expected_cmd.split(",") if item]
        if command != expected:
            print(
...
def _build(args, policy: CompositionPolicy) -> int:
    if not args.layer:
    ...
    output = Path(workspace.name) / "layout"
    ...
    for index, source_layer in enumerate(args.layer):
        blob, layer_digest = ingest_layer(source_layer, blobs, index)
        entries, diff_id = validate_layer(blob)
        apply_layer_state(
            entries,
            effective_state,
    ...
    cfg: dict = {
        "Env": [f"{k}={v}" for k, v in kv_pairs(args.env).items()],
    }
    if args.cmd:
    ...
    image_config = {
        ...
        "history": [
            {
                "created": REPRODUCIBLE_CREATED,
                "created_by": f"scratch_image.py (layer {i + 1}/{len(diff_ids)})",
            }
    ...
    if final_output.exists():
        if final_output.is_symlink() or not final_output.is_dir() or any(final_output.iterdir()):
            raise UnsafeInputError(f"output changed while building: {final_output}")
        final_output.rmdir()
...
def main() -> int:
    ...
    build = commands.add_parser("build", help="assemble an OCI layout from layer tars")
    build.add_argument("--output", required=True, type=Path)
    build.add_argument("--layer", action="append", default=[], type=Path,
                       help="tar file to add as a layer (in order; first = bottom)")
    build.add_argument("--cmd", help="comma-separated argv for Cmd")
    build.add_argument("--env", action="append", default=[], help="KEY=VAL")
    build.add_argument("--label", action="append", default=[], help="KEY=VAL")
    build.add_argument("--user", help="User field (e.g. root, 1000, 1000:1000)")
    build.add_argument("--port", action="append", default=[], help="e.g. 22/tcp")
    build.add_argument("--arch", default="amd64")
    build.add_argument("--os", default="linux")
    validate = commands.add_parser("validate", help="re-check a finished OCI layout")
    validate.add_argument("layout", type=Path)
    validate.add_argument("--expected-cmd",
                          help="comma-separated argv the config's Cmd must equal")
    for parser in (build, validate):
        parser.add_argument("--base-layer-count", default=0, type=int,
                            help="number of trusted base layers before the policy applies")
        parser.add_argument("--policy", type=Path,
                            help="composition policy file; omitted, nothing is enforced")
    args = ap.parse_args()

    policy = load_policy(args.policy) if args.policy else CompositionPolicy()
    if args.command == "validate":
        return _validate(args, policy)
