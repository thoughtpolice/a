...
`s3.put_object.after_commit`. It maps typed `S3Fault` values into `s3s` errors
at the handler. Entry points precede storage locks and streaming object body
consumption. Post-commit points release the storage lock before awaiting, so
another request can observe the committed write while its original response
is paused or fails.

Use named failpoints for targeted scenarios such as the first two PUTs
returning `SlowDown`. Add `buggify!` branches where broader stress is useful:
tiny response chunks, an extra yield, a shorter batch, or an uncommon error.
Both mechanisms stay scoped to the same mems3 instance.

The repeatable `--failpoint NAME=PLAN` option validates names and typed errors
before starting the server; `--list-failpoints` discovers the available names.
`--buggify` enables request yield and `SlowDown` branches, with `--fault-seed`
and probability options controlling the run. Its Buck resource helpers forward
these settings to private server processes. For example:
...
For whole-service exploration, mems3 also provides `--auto-buggify`: its
`storage-v1` policy combines throttling, bounded delays, yields, and errors after
committed writes. Request counts define optional warmup and recovery phases.
Each active request receives a separate `Buggify` controller whose seed derives
from the campaign seed and its ordinal; that controller follows the request
through commit. This is an application of instance ownership: completion order
cannot move one request's random decisions into another request's stream.

```console
buck2 run tilde//aseipp/mems3:mems3 -- \
    --fault-seed 42 --auto-buggify --chaos-requests 2000 --chaos-trace
```
