...
use faultline::{Action, BuggifyConfig, Injector, Probability};
...
#[tokio::test]
async fn buggify_returns_slow_down_until_the_controller_is_disabled() {
    let (faults, service) = fault_fixture();
    let key = "/celld/buggify";
    put(&service, key, "healthy").await;
    let guard = faults
        .buggify()
    ...
    let response = call(&service, request(Method::GET, key, Body::empty())).await;
    expect_error(response, StatusCode::SERVICE_UNAVAILABLE, "SlowDown").await;
    for name in ["s3.request.yield", "s3.request.slow_down"] {
        let sites = guard.snapshot();
        let site = sites.iter().find(|site| site.name == name).unwrap();
        assert!(site.activated);
        assert_eq!(site.hits, 1);
        assert_eq!(site.fired, 1);
    }

    faults.buggify().disable();
    assert_eq!(get_text(&service, key).await, "healthy");
    put(&service, key, "recovered").await;
    assert_eq!(get_text(&service, key).await, "recovered");
}
