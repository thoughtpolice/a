...
fn automatic_campaign_parses_counts_and_rejects_ambiguous_configuration() {
    ...
    assert_eq!(
        parse(&["--auto-buggify"]).unwrap().unwrap().chaos,
        Some(crate::chaos::ChaosConfig::default())
    );
    // Maximum values are accepted without overflowing phase calculations.
    assert!(parse(&["--auto-buggify", "--chaos-requests=18446744073709551615"]).is_ok());
    assert!(
        parse(&[
            "--auto-buggify",
    ...
    for (args, expected) in [
        (vec!["--chaos-trace"], "require --auto-buggify"),
        (vec!["--chaos-warmup-requests=0"], "require --auto-buggify"),
        (vec!["--chaos-requests=1"], "require --auto-buggify"),
        (
            vec!["--auto-buggify", "--chaos-requests=0"],
            "must be positive",
        ),
        (
            vec!["--auto-buggify", "--chaos-requests=-1"],
        ...
        (
            vec!["--auto-buggify", "--chaos-requests"],
        ...
        (vec!["--auto-buggify", "--buggify"], "incompatible"),
        (
            vec!["--auto-buggify", "--failpoint=s3.get_object.before=off"],
            "incompatible",
        ),
        (
            vec!["--auto-buggify", "--buggify-firing=10"],
            "require --buggify",
        ),
        (vec!["--auto-buggify=true"], "unknown argument"),
    ] {
...
#[test]
fn parses_typed_fault_plans_seed_and_buggify_options_in_both_forms() {
    use crate::faults::S3Fault;
    use faultline::{Action, Probability};
    ...
    assert_eq!(
        ...
        [
            (
                ...
                Action::Return(S3Fault::SlowDown).times(2).into()
            ),
            (
                "s3.put_object.after_commit".to_owned(),
                Action::Return(S3Fault::InternalError).into()
            ),
        ]
    );
    assert_eq!(args.fault_seed, Some(u64::MAX));
    let config = args.buggify.unwrap();
    assert_eq!(config.activation, Probability::ALWAYS);
    assert_eq!(config.firing, Probability::new(0.125).unwrap());
    let args = parse(&["--fault-seed", "0", "--buggify"]).unwrap().unwrap();
    assert_eq!(args.fault_seed, Some(0));
    assert_eq!(args.buggify, Some(faultline::BuggifyConfig::default()));
}
...
fn rejects_fault_configuration_that_would_silently_weaken_a_test() {
    for (args, expected) in [
        ...
        (vec!["--buggify-firing=100"], "require --buggify"),
        (vec!["--buggify-activation=100"], "require --buggify"),
        (
            vec!["--buggify", "--buggify-firing"],
            "requires a percentage",
        ),
        (
            vec!["--buggify", "--buggify-activation"],
            "requires a percentage",
        ),
        (vec!["--buggify=true"], "unknown argument"),
    ] {
        let error = parse(&args).unwrap_err();
        assert!(error.contains(expected), "{args:?}: {error}");
    }
    for flag in ["--buggify-activation", "--buggify-firing"] {
        for percent in ["NaN", "inf", "-1", "100.01", "", "garbage"] {
            let error = parse(&["--buggify", flag, percent]).unwrap_err();
            assert!(
                error.contains("expected a finite percentage"),
                "{flag} {percent}: {error}"
            );
        }
    }
...
fn accepts_both_option_forms_and_repeated_buckets() {
    ...
    let args = parse(&["--listen=[::1]:1234"]).unwrap().unwrap();
