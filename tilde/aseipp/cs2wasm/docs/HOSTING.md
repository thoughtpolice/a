...
units, and free of control characters. A module can declare at most 256 imports.
Methods cannot have bodies, other attributes (including return attributes),
optional parameters, or by-reference parameters.

## Multi-frame example

Inside `nix-shell`, from the repository root:
...
resets its fuel, call depth, allocation accounting, and fault code at every
exported entry. These budgets do not meter time inside a host callback or limit
