...
try {
  ...
  // Copy only the executable. No SDK, Roslyn libraries, or reference assemblies
  // are available to it, and it cannot find external tools through PATH.
  const compilerEnvironment = {
    ...process.env,
    PATH: '',
    DOTNET_ROOT: path.join(scratch, 'no-sdk'),
    DOTNET_ROOT_X64: path.join(scratch, 'no-sdk'),
  ...
  function checkSuite(suite, module) {
  ...
  for (const suite of suites) {
    ...
    nativeChecks += checkSuite(suite, module);
  }
  ...
  if (withDifferential) {
    // The independent .NET oracle needs the pinned SDK in the runner's environment.
    const result = spawnSync(
      process.execPath,
      [path.join(root, 'tests/differential.mjs'), path.resolve(nativeCompiler)],
