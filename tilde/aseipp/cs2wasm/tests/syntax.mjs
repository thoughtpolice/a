...
export function checkSyntax(exports, assert) {
  ...
  function expect(name, args, expected) {
    assert.equal(exports['Tests.Syntax.' + name](...args), expected, name);
    checks++;
  }
  function trap(name, args, fault) {
    assert.throws(
      () => exports['Tests.Syntax.' + name](...args),
      WebAssembly.RuntimeError,
      name,
    );
    assert.equal(exports.__fault.value, fault, name + ' fault');
    checks++;
    expect('AddAssignment', [20, 22], 42);
