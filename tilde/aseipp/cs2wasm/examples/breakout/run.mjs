...
for (; ticks < 20_000 && host.snapshot().status === STATUS.playing; ticks++) {
  ...
  if ((ticks + 1) % 2400 === 0) {
    ...
    console.log(
      `${
        ((ticks + 1) * WORLD.stepSeconds).toFixed(0)
      }s: score ${state.score}, ${remaining} ${
        remaining === 1 ? "brick" : "bricks"
      } left.`,
    );
  }
}
...
console.log(
  `Won: score ${final.score}, all 32 bricks cleared in ${ticks} ticks (${
    (ticks * WORLD.stepSeconds).toFixed(2)
  } simulated seconds).`,
);
