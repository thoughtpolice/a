...
let previousTime = 0;
let liveBricks = 0;
const brickColors = ["#83d9c4", "#80b9e6", "#b9a0e1", "#efb987"];
...
function refreshControls() {
  ...
  title.textContent = status === STATUS.won
    ? "Board cleared."
    : status === STATUS.lost
    ? "Out of bounds."
    : started
...
function togglePlay() {
  if (!host || host.status() !== STATUS.playing) return;
  running = !running;
  started = true;
  elapsed = 0;
...
function tick() {
  ...
  if (direction !== 0) {
    targetX = host.entity(host.paddleHandle).x +
      direction * 600 * WORLD.stepSeconds;
  }
  const status = host.step(WORLD.stepSeconds, targetX);
  if (status !== STATUS.playing) {
    pause();
    statusText.textContent = status === STATUS.won
...
function reportError(error) {
  ...
  statusText.textContent =
    `Tick failed: ${error.message}. Fault ${host.lastFault()}. Pending changes were discarded.`;
}
...
function drawEntity(entity) {
  ...
  if (!paddle) liveBricks++;
  const halfWidth = paddle ? WORLD.paddleHalfWidth : WORLD.brickHalfWidth;
  const halfHeight = paddle ? WORLD.paddleHalfHeight : WORLD.brickHalfHeight;
  context.fillStyle = paddle
    ? "#eef4fa"
    : brickColors[Math.floor((entity.handle - 100) / 8) % brickColors.length];
  context.fillRect(
    entity.x - halfWidth,
    entity.y - halfHeight,
    halfWidth * 2,
...
function draw() {
  ...
  liveBricks = 0;
  host.forEachEntity(drawEntity);
  score.textContent = host.score();
  remaining.textContent = `${liveBricks} ${
    liveBricks === 1 ? "brick" : "bricks"
  } left`;
}

function frame(time) {
  const seconds = previousTime === 0
    ? 0
...
rollback.addEventListener("click", () => {
  ...
  try {
    ...
    statusText.textContent = error instanceof WebAssembly.RuntimeError &&
        host.lastFault() === 6 &&
        unchanged
