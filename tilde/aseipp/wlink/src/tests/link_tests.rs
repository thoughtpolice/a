...
const GC_CONSUMER: &str = include_str!("../../tests/gc_consumer.wat");
            ...
            Payload::TypeSection(reader) => {
                for group in reader {
                    for ty in group.unwrap().into_types() {
                        // Heap types keep their slot in the index space
                        // but are never an import's signature.
            ...
            Payload::ImportSection(reader) => {
                for import in reader.into_imports() {
                    ...
                    let ty = types[ty as usize]
                        .as_ref()
                        .expect("an import's type is a function type");
                    shape.imports.push((
...
#[test]
fn heap_types_in_a_recursive_group_do_not_shift_the_added_types() {
    // A recursive type group is one type-section entry holding several types.
    // The types the linker adds after it, for the package's imports and its
    // adapters, must be numbered past all of them; counting entries instead
    // bound every synthesized call to the wrong signature, and the output
    // failed to validate.
    let linked = wlink::link(&[
        input("producer", SCALAR_PRODUCER),
        input("consumer", GC_CONSUMER),
    ])
    .expect("a consumer with GC heap types links against a scalar producer");
    let shape = shape(&linked.module);
    ...
    assert_eq!(
        ...
        "the unsatisfied import keeps its own signature"
    );
    assert_eq!(shape.exports.get("run"), Some(&ExternalKind::Func));
    assert_eq!(
        shape.defined_functions, 3,
        "add binds directly; only the remaining import gets its wrapper"
    );
}
