...
fn function_type(types: &mut TypeSection, params: &[CoreType], results: &[CoreType]) -> u32 {
    let index = types.len();
    types.ty().function(
...
pub fn merge(plan: &Plan) -> Result<Vec<u8>> {
    ...
    for (index, import) in plan.imports.iter().enumerate() {
        ...
        let ty = function_type(&mut types, &signature.params, &signature.results);
    ...
    for (offset, addition) in additions.iter().enumerate() {
        let ty = function_type(&mut types, &addition.params, &addition.results);
