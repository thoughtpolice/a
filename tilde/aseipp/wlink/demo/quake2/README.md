...
That serves a 51 MB package holding the browser host, the linked module and
the pak at `baseq2/pak0.pak`. A page is not a terminal, so it starts at
1280x720 scaled to fill the window at 16:9, rather than the 320x240 a
terminal gets. The size rides in the package's own arguments, and `?arg=`
replaces those rather than adding to them, so `?arg=%2Bmap&arg=demo1` goes
straight
into the first level at the engine's default size; a link that wants both
spells out `+set sw_mode -1`, `+set vid_width 1280` and `+set vid_height
720` as well. Because the package carries a copy of the pak, it is never
uploaded to a shared cache, as the pak itself is not. `:quake2-web-headless`
replays it with the native runner's options, and `:test-quake2-e2e-web` and
...
the process ends through it. The engine's own mode table stops at 1600x1200
and is 4:3 the whole way down, so the shim adds `sw_mode -1`, which takes its
size from the `vid_width` and `vid_height` cvars and is bounded by what the
renderer's scanline tables and the console's input line can hold; that is how
the browser package asks for 720p. The engine draws the status bar, the menus
and the console text a pixel to a pixel at every mode, so they take up less of
the screen the larger it gets. `hunk.c` replaces the engine's model allocator
