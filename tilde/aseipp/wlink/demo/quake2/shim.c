...
// The engine's mode table stops at 1600x1200 and is 4:3 the whole way down,
// and a browser window is neither. This mode takes its size from two cvars
// instead, which is how every Quake II since has spelled a resolution the
// table does not have.
#define CUSTOM_MODE (-1)

// The renderer indexes its per-scanline tables by MAXHEIGHT, and the console
// draws its input line one character per eight pixels into the 256-byte
// buffer a typed command lives in, so a wider frame writes past the end of
// it. A mode the table does have is the engine's business to bound; these
// two are the port's, and reach the renderer from a cvar anyone can set.
#define CUSTOM_MAX_WIDTH 2048

static bool custom_mode(int* width, int* height) {
  float w = ri.Cvar_Get("vid_width", "640", CVAR_ARCHIVE)->value;
  float h = ri.Cvar_Get("vid_height", "480", CVAR_ARCHIVE)->value;
  if (!(w >= 320 && w <= CUSTOM_MAX_WIDTH && h >= 240 && h <= MAXHEIGHT)) return false;
  *width = (int)w;
  *height = (int)h;
  return true;
}
...
rserr_t SWimp_SetMode(int* pwidth, int* pheight, int mode, qboolean fullscreen) {
  ...
  int new_width;
  int new_height;
  bool known = mode == CUSTOM_MODE ? custom_mode(&new_width, &new_height)
                                   : (bool)ri.Vid_GetModeInfo(&new_width, &new_height, mode);
  if (!known) {
