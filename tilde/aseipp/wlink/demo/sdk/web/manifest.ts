...
export interface Manifest {
  ...
  /** How a frame is shaped on screen: the terminal's 4:3 box, or square pixels. */
  aspect: "4:3" | "frame";
...
export function parseManifest(value: unknown): Manifest {
  ...
  if (aspect !== "4:3" && aspect !== "frame") fail(`aspect ${aspect} is not 4:3 or frame`);
  ...
  const mounts = rawMounts.map((mount, index) => {
    if (typeof mount !== "object" || mount === null) fail(`mounts[${index}] is not an object`);
  ...
  return {
    ...
    framesPerSecond: asCount(field(record, "frames_per_second"), "frames_per_second"),
    aspect,
    option,
    args,
    mounts,
