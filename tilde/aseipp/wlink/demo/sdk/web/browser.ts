...
function options(): Options {
  ...
  return {
    ...
    renderer: renderer === "webgpu" || renderer === "canvas2d"
      ? renderer
      : "auto",
...
async function boot(): Promise<void> {
  ...
  try {
    ...
    moduleBytes = await fetchBytes(url(manifest.module), (loaded, total) => {
      page.say(
        manifest.title,
        `Loading the game… ${bytesLabel(loaded, total)}`,
        null,
      );
    });
    mounts = [];
    for (const mount of manifest.mounts) {
      const bytes = await fetchBytes(url(mount.file), (loaded, total) => {
        page.say(
          manifest.title,
          `Loading ${mount.path}… ${bytesLabel(loaded, total)}`,
          null,
        );
      });
  ...
  const sinks: Sinks = {
    ...
    capturePointer: (captured) => {
      ...
      else if (document.pointerLockElement === page.canvas) {
  ...
  page.say(
    manifest.title,
    "Sound and the keyboard need a click to start.",
    "Click to start",
  );
  ...
  async function startAudio(): Promise<void> {
    try {
      ...
      node = new AudioWorkletNode(context, "console-sink", {
        outputChannelCount: [2],
      });
      node.port.onmessage = (event: MessageEvent) => {
        if (event.data?.type === "stats") {
          audio.underruns = event.data.underruns ?? 0;
        }
      };
...
class Renderers {
  ...
  async prepare(): Promise<void> {
    if (this.preferred !== "canvas2d") {
      ...
      if (this.preferred === "webgpu") {
        this.warn("WebGPU is unavailable; drawing with canvas 2D");
      }
    }
    this.fallBack();
  }
  ...
  draw(runner: Runner): void {
    // A frame at nearly the canvas's own size is filtered and pixel art is
    // not. The renderer may have been replaced by the fallback since the
    // manifest was read, so the choice is pushed rather than set once.
    if (this.renderer) this.renderer.smooth = this.aspect === "fill";
...
function attachInput(
  ...
  canvas.addEventListener("keydown", (event) => {
    ...
    if (key !== undefined && !event.ctrlKey && !event.metaKey) {
      event.preventDefault();
    }
    ...
    if (
      ...
      if (code >= 0x20 && code !== 0x7f) {
        input.typeText(new TextEncoder().encode(event.key));
      }
    }
  });
  ...
  canvas.addEventListener("pointermove", (event) => {
    if (document.pointerLockElement === canvas) {
      ...
      input.setPosition(
        Math.max(
          0,
          Math.min(runner.state.width - 1, input.mouseX + event.movementX),
        ),
        Math.max(
          0,
          Math.min(runner.state.height - 1, input.mouseY + event.movementY),
        ),
      );
      return;
    }
  ...
  const buttons = (event: PointerEvent) => {
    if (document.pointerLockElement === canvas) {
      input.setButtons(event.buttons & 7);
    } else {
      const at = toFrame(event);
      if (at) input.moveMouse(at[0], at[1], event.buttons & 7, 0);
  ...
  function tick(now: number): void {
    ...
    if (now - shownAt >= 500) {
      ...
      page.status.textContent =
        `${runner.state.frames} frames · ${fps.toFixed(0)} fps · ` +
