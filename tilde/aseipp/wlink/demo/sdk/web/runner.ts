...
function isResponseLike(
  source: ModuleSource,
): source is Response | Promise<Response> {
...
export class Runner {
  ...
  static async instantiate(
    ...
    const imports = makeImports(
      ...
      () => {
        if (!resolved) {
          throw new GuestTrap(
            "the HAL was called before instantiation finished",
          );
        }
        return resolved;
      },
  ...
  private queueInput(frames: number): void {
    ...
    this.cursor = this.state.input.deliverScript(
      this.script,
      this.cursor,
      frames,
    );
  ...
  gameMemoryBytes(): number {
    return this.exports.gameMemory?.buffer.byteLength ?? 0;
  }
