...
export function fileName(bytes: Uint8Array): string | null {
  let start = 0;
  while (bytes.length - start >= 2 && bytes[start] === 0x2e && bytes[start + 1] === 0x2f) {
    start += 2;
  }
  ...
  for (let i = 0; i <= path.length; i++) {
    ...
    if (segment === 2 && path[i - 2] === 0x2e && path[i - 1] === 0x2e) return null;
    segment = 0;
  }
...
export class Vfs {
  ...
  private childOf(entry: Entry, dir: string): boolean {
    ...
    let rest = entry.name;
    if (dir !== "") {
      if (!entry.name.startsWith(dir) || entry.name[dir.length] !== "/") return false;
  ...
  private children(dir: string): number[] {
    ...
    indexes.sort((a, b) => compareCodePoints(this.entries[a].name, this.entries[b].name));
    return indexes;
  }

  private flush(index: number): void {
    const entry = this.entries[index];
    if (!this.hasMirror() || entry.directory || entry.removed || !entry.dirty) return;
    entry.dirty = false;
    try {
  ...
  mountReadonly(path: string, data: Uint8Array): boolean {
    ...
    if (name === null || this.find(name) >= 0 || !this.ensureParents(name)) return false;
   ...
   */
  loadEntry(name: string, directory: boolean, data: Uint8Array | null): string | null {
  ...
  open(path: Uint8Array, write: boolean): number {
    ...
    if (index < 0 && write && this.ensureParents(name)) index = this.add(name, false);
    if (index < 0) return -1;
    const entry = this.entries[index];
  ...
  readAt(fd: number, offset: bigint, length: number): { status: number; data: Uint8Array } {
    const index = this.handleEntry(fd);
  ...
  remove(path: Uint8Array): number {
    ...
    if (index < 0) return -1;
    const entry = this.entries[index];
    if (entry.readonly || (entry.directory && this.hasChildren(name))) return -1;
  ...
  rename(fromPath: Uint8Array, toPath: Uint8Array): number {
    ...
    if (target >= 0) {
      ...
      if (this.entries[target].openCount > 0) this.entries[target].removed = true;
    ...
    if (directory) {
      for (const entry of this.entries) {
        if (entry.name === "" || entry.removed || !entry.name.startsWith(prefix)) continue;
