...
function parseArguments(argv: string[], manifestOption: string | null): Options {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (manifestOption !== null && option === `--${manifestOption}`) {
      ...
      options.framesPerSecond = parseCount(value, "invalid --frames-per-second");
    ...
    else if (option === "--unix-time") options.unixTime = parseUnsigned(value, "--unix-time");
...
async function run(options: Options, io: Io): Promise<number> {
  ...
  if (options.framesPerSecond !== null) manifest.framesPerSecond = options.framesPerSecond;

  const modulePath = options.module ??
    (options.packageDir !== null ? `${options.packageDir}/${manifest.module}` : null);
  ...
  if (options.saveDir !== null && options.noSave) throw new UsageError("--save-dir");
  ...
  if (manifest.option !== null) {
    ...
    if (replacement !== undefined) mounts.set(manifest.mounts[0].path, replacement);
  }
  ...
  const wav = options.wav !== null ? new WavWriter(wavOutput(options.wav)) : null;
  const audioSink: AudioSink = wav === null ? { chunk: () => {}, silence: () => {} } : {
  ...
  const record = options.record !== null
    ? Deno.openSync(options.record, { write: true, create: true, truncate: true })
    : null;
  if (record !== null) {
    runner.setRecorder(
  ...
  runner.state.args = [manifest.name, ...(options.args.length > 0 ? options.args : manifest.args)];
...
function writeAll(stream: { writeSync(bytes: Uint8Array): number }, text: string): void {
