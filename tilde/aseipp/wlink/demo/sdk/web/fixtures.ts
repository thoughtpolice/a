...
function sleb(value: number): number[] {
  ...
  while (more) {
    ...
    rest >>= 7;
    if ((rest === 0 && (byte & 0x40) === 0) || (rest === -1 && (byte & 0x40) !== 0)) more = false;
...
export function fixtureModule(spec: FixtureSpec = {}): Uint8Array<ArrayBuffer> {
  ...
  const initBody = spec.init === "set-rate-70" ? [0x41, ...sleb(70), 0x10, 0x01, 0x1a, END] : [END];
  const frameBody = {
  ...
  const bodies = [reallocCode, [0x00, ...initBody], [0x00, ...frameBody], [0x00, END]];
  ...
  for (const fn of MEMORY_IMPORTS) {
    exports.push([...name(`wlink:import:${HAL_MODULE}#${fn}:memory`), 0x02, ...uleb(0)]);
  }
  for (const fn of REALLOC_IMPORTS) {
    exports.push([...name(`wlink:import:${HAL_MODULE}#${fn}:realloc`), 0x00, ...uleb(2)]);
  }
