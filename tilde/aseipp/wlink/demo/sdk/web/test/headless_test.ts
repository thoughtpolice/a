...
function script(text: string): string {
  const path = Deno.makeTempFileSync({ prefix: "console-script-", suffix: ".txt" });
...
Deno.test({
  ...
  fn: async () => {
    ...
    const capture = Deno.makeTempFileSync({ prefix: "console-frame-", suffix: ".ppm" });
    const io = new Output();
    const code = await main([
      "--module",
      CONTRACT as string,
    ...
    for (
      ...
      assert(io.out.includes(`PASS sdk ${stage}`), `PASS sdk ${stage} is missing`);
    }
    const fields = summary(io.out);
