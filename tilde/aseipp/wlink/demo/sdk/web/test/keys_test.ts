...
function keysFromWit(): string[] {
  const wit = Deno.readTextFileSync(new URL("../../wit/sdk.wit", import.meta.url));
...
Deno.test("every key is reachable from a KeyboardEvent.code", () => {
  ...
  const missing = KEY_NAMES.map((_, ordinal) => ordinal).filter((o) => !reached.has(o));
  assertEquals(missing.map((o) => KEY_NAMES[o]), []);
  for (const [code, ordinal] of Object.entries(CODE_TO_KEY)) {
    assert(ordinal >= 0 && ordinal < KEY_COUNT, `${code} maps outside the enum`);
  }
});
