...
function names(entries: Listing[] | null): string[] {
  ...
  return entries.map((entry) =>
    `${entry.name} ${entry.size} ${entry.directory}`
  );
}
...
Deno.test("names are ordered as the host orders their bytes", () => {
  ...
  assert(
    compareCodePoints("￿", "\u{10000}") < 0,
    "the supplementary planes come last",
  );
  assert(
    "￿" < "\u{10000}" === false,
    "which is not what UTF-16 comparison says",
  );
...
Deno.test("directories, as the SDK contract exercises them", () => {
  ...
  assertEquals(names(vfs.listDirectory(p(""))), [
    "contract.tmp 5 false",
    "doom2.wad 64 false",
  ]);
  assertEquals(
    names(vfs.listDirectory(p("."))),
    names(vfs.listDirectory(p(""))),
  );
  ...
  assertEquals(names(vfs.listDirectory(p("contract-implicit"))), [
    "deep 0 true",
  ]);

  assertEquals(
    vfs.rename(p("contract-dir/sub/a.txt"), p("contract-dir/sub/c.txt")),
    0,
  );
  ...
  assertEquals(
    vfs.rename(p("contract-dir/sub/c.txt"), p("contract-dir/sub/b.txt")),
    0,
  );
  assertEquals(names(vfs.listDirectory(p("contract-dir/sub"))), [
    "b.txt 3 false",
  ...
  assertEquals(names(vfs.listDirectory(p("contract-moved/deep"))), [
    "file.txt 1 false",
  ]);
  assertEquals(
    vfs.rename(p("contract-moved"), p("contract-moved/deep/inside")),
    -1,
  );
...
Deno.test("a renaming that would not fit, and one into its own subtree", () => {
  ...
  assertEquals(names(vfs.listDirectory(p("dddd"))), [
    `${"x".repeat(250)} 4 false`,
  ]);
...
Deno.test("a prepared directory is read back and removed", () => {
  ...
  assertEquals(
    vfs.loadEntry("preexisting/hello.txt", false, encoder.encode("hi")),
    null,
  );
  assertEquals(names(vfs.listDirectory(p("preexisting"))), [
    "hello.txt 2 false",
  ]);
  ...
  assertEquals(
    vfs.loadEntry("doom2.wad", true, null),
    "a mounted asset is in the way, skipped",
  );
});
...
Deno.test("writing far past the end zero-fills the gap", () => {
  ...
  assertEquals(
    vfs.writeAt(fd, BigInt(64 * 1024 * 1024), encoder.encode("z")),
    -1,
  );
