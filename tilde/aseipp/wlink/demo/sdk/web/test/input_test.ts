...
import { InputState, MAX_EVENTS, MAX_TEXT, parseScript, Recorder, textBytes } from "../input.ts";
...
Deno.test("the scripts the end-to-end suites replay", () => {
  ...
  assertEquals(doom[0], { frame: 100, kind: "key", key: keyCode("w"), pressed: true });
  assertEquals(doom[5], { frame: 220, kind: "key", key: keyCode("space"), pressed: false });

  const contract = parseScript(bytes(CONTRACT_SCRIPT));
  assertEquals(contract.length, 8);
  assertEquals(contract[6], { frame: 0, kind: "mouse", x: 5, y: 6, buttons: 1, wheel: 0 });
  assertEquals(contract[7], { frame: 0, kind: "text", text: bytes("hi☃") });
});

Deno.test("comments and blank lines are skipped", () => {
  const script = parseScript(bytes("# a recording\n\n10 a down\n\n#\n20 a up\n"));
...
Deno.test("what a script may not say", () => {
  ...
  const invalid = new Uint8Array([0x31, 0x20, 0x74, 0x65, 0x78, 0x74, 0x20, 0xff, 0x0a]);
...
Deno.test("a text payload keeps its spaces and loses its line ending", () => {
  ...
  assertEquals(script[0], { frame: 5, kind: "text", text: bytes(" two  spaces ") });
});

Deno.test("the pointer accumulates motion and notches until it is read", () => {
  const input = new InputState();
  input.moveMouse(10, 20, 0, 0);
  assertEquals(input.readMouse(), { x: 10, y: 20, dx: 0, dy: 0, buttons: 0, wheel: 0 });
  input.moveMouse(13, 24, 1, -1);
  input.moveMouse(15, 24, 1, -1);
  assertEquals(input.readMouse(), { x: 15, y: 24, dx: 5, dy: 4, buttons: 1, wheel: -2 });
  assertEquals(input.readMouse(), { x: 15, y: 24, dx: 0, dy: 0, buttons: 1, wheel: 0 });
  input.addMotion(-3, 7);
  assertEquals(input.readMouse(), { x: 15, y: 24, dx: -3, dy: 7, buttons: 1, wheel: 0 });
});
...
Deno.test("a recording replays as the same script", () => {
  const script = parseScript(bytes(CONTRACT_SCRIPT + "3 mouse 9 9 0 -1\n4 text  hi\n"));
  const lines: string[] = [];
  const input = new InputState();
  ...
  assertEquals(parseScript(bytes(recorded.split("\n").slice(1).join("\n"))), script);
});
