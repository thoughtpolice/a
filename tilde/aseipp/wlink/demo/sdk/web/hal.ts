...
export function makeImports(
  ...
  const raw: Imports = {
    ...
    "present": (width: number, height: number, ptr: number, len: number): void => {
      const w = width >>> 0;
      const h = height >>> 0;
      const size = len >>> 0;
    ...
    "set-frame-rate": (hz: number): number => {
      const rate = hz >>> 0;
      if (state.frames === 0 && rate >= 1 && rate <= 1000) state.framesPerSecond = rate;
    ...
    "read-mouse": (result: number): void => {
      ...
      const fields = [mouse.x, mouse.y, mouse.dx, mouse.dy, mouse.buttons, mouse.wheel];
      for (let i = 0; i < fields.length; i++) mem.store32(result + 4 * i, fields[i]);
    },

    "capture-pointer": (captured: number): number => sinks.capturePointer((captured >>> 0) !== 0),
    ...
    "file-read-at": (fd: number, offset: bigint, length: number, result: number): void => {
      const binding = bind("file-read-at");
      const read = state.vfs.readAt(fd >>> 0, BigInt.asUintN(64, offset), length >>> 0);
    ...
    "file-write-at": (fd: number, offset: bigint, ptr: number, len: number): number => {
    ...
    "file-rename": (ptr: number, len: number, toPtr: number, toLen: number): number => {
    ...
    "file-create-directory": (ptr: number, len: number): number =>
      state.vfs.createDirectory(bind("file-create-directory").mem.range(ptr, len)),
