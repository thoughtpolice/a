...
export class Mem {
  ...
  span(ptr: number, byteLength: number): Uint8Array {
    const start = ptr >>> 0;
    const bytes = this.view();
    if (byteLength < 0 || start + byteLength > bytes.length) {
      throw new GuestTrap(OUT_OF_BOUNDS);
    }
...
export class Binding {
  ...
  alloc(align: number, len: number): number {
    ...
    if (!this.realloc) {
      throw new GuestTrap(
        `${this.name} has no realloc to allocate its result with`,
      );
    }
...
export interface Exports {
  ...
  /** Absent for a game that keeps its state on the GC heap, with no linear memory. */
  readonly gameMemory: WebAssembly.Memory | null;
...
export function discoverExports(exports: WebAssembly.Exports): Exports {
  const fn = (name: string): (...args: number[]) => number | void => {
    ...
    if (typeof value !== "function") {
      fail(`the module does not export a function ${name}`);
    }
  ...
  // A game compiled to the GC heap (gameplayc) has no memory to export; a
  // game that exports something else under the name is still wrong.
  const gameMemory = "game:memory" in exports ? memory("game:memory") : null;
