...
export type ScriptEvent =
  ...
  | { frame: number; kind: "mouse"; x: number; y: number; buttons: number; wheel: number }
...
export class InputState {
  ...
  moveMouse(x: number, y: number, buttons: number, wheel: number): void {
    ...
    this.mouseChanged = this.mouseChanged || !this.mouseSeen || x !== this.mouseX ||
   ...
   */
  deliverScript(script: readonly ScriptEvent[], cursor: number, frames: number): number {
    while (
...
export function parseScript(bytes: Uint8Array): ScriptEvent[] {
  ...
  while (start <= bytes.length) {
    ...
    if (kind === "mouse") {
      ...
      if (x === null || y === null || buttons === null || wheel === null || buttons > 7) {
        throw new Error(SCRIPT_ERROR);
      }
      ...
      const payload = at < 0 ? new Uint8Array(0) : trimEol(line.subarray(at + 5));
      if (payload.length === 0 || !validUtf8(payload)) throw new Error(SCRIPT_ERROR);
      ...
      if (key < 0 || (action !== "down" && action !== "up")) throw new Error(SCRIPT_ERROR);
...
function parseInteger(field: Uint8Array, signed: boolean): number | null {
  let at = 0;
  let negative = false;
  if (signed && at < field.length && (field[at] === 0x2d || field[at] === 0x2b)) {
...
function trimEol(payload: Uint8Array): Uint8Array {
  let end = 0;
  while (end < payload.length && payload[end] !== 0x0d && payload[end] !== 0x0a) end++;
...
export class Recorder {
  ...
  frame(frames: number, input: InputState): void {
    for (const event of input.frameEvents()) {
      ...
      this.sink(`${frames} ${KEY_NAMES[event.key]} ${event.pressed ? "down" : "up"}\n`);
    }
