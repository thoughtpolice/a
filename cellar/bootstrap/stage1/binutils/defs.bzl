...
load("@cellar//bootstrap/stage1/binutils:sources.bzl", "BINUTILS_HEADERS", "COMMON_HEADERS")
...
def binutils_stage(
        ...
        bfd_test_object,
        action_env = {},
        libc_declares_basename = True,
        source_alias = None):
    """Declares the native binutils 2.30 graph for one predecessor toolchain.

    The calling package must declare the `binutils-2.30` source target, whose
    subtargets are the pinned release paths in FILES. `tools` maps bash, bison,
    cat, flex, gawk, mkdir, rm and sed to the generator executables.

    musl 1.1.24's <string.h> declares basename, and musl 1.2.5's does not; for
    the latter, pass `libc_declares_basename = False` so that libiberty.h
    declares it.
    ...
    def basename_config(on, values):
        if libc_declares_basename:
            return on, values
        return [name for name in on if name != "HAVE_DECL_BASENAME"], values | {"HAVE_DECL_BASENAME": "0"}

    libiberty_on, libiberty_values = basename_config(LIBIBERTY_ON, LIBIBERTY_VALUES)
    ...
    exact_patch(
        ...
        src = ":binutils-2.30[libiberty/make-temp-file.c]",
        patch = "cellar//bootstrap/stage1/binutils:patches/tmpdir.patch",
    )
    ...
    c_object(
        name = "zlib-tables.o",
        src = "cellar//bootstrap/stage1/binutils:generators/zlib-tables.c",
    ...
    c_object(
        ...
        src = "cellar//bootstrap/stage1/binutils:tests/bfd-headers.c",
    ...
    bfd_on, bfd_values = basename_config(BFD_ON, BFD_VALUES)
    ...
    c_object(
        name = "bfd-test.o",
        src = "cellar//bootstrap/stage1/binutils:tests/bfd.c",
        flags = cflags,
        headers = [
            ":bfd-source",
            libc_headers,
    ...
    opcodes_on, opcodes_values = basename_config(OPCODES_ON, OPCODES_VALUES)
    ...
    [
        exact_patch(
            name = name,
            src = (":binutils-2.30[opcodes/i386-gen.c]" if i == 0 else ":" + GEN_PATCHES[i - 1]),
            patch = "cellar//bootstrap/stage1/binutils:patches/" + name + ".patch",
        )
    ...
    c_object(
        name = "opcodes-test.o",
        src = "cellar//bootstrap/stage1/binutils:tests/opcodes.c",
        flags = cflags,
        headers = [
            ":opcodes-source",
            ":bfd-headers",
