...
int main(int argc, char **argv) {
    ...
    if (argc==3) {
        ...
        section=bfd_get_section_by_name(b,".text"); CHECK(section && bfd_section_size(b,section)>0);
    ...
    CHECK(bfd_set_section_alignment(b,section,3) && bfd_set_section_size(b,section,sizeof(input)) && bfd_set_section_vma(b,section,address));
    {
    ...
    CHECK(bfd_section_vma(b,section)==address && bfd_section_size(b,section)==sizeof(input));
