...
Example usage:
./simple-patch input before_pattern_file after_pattern_file output

Cellar changes: immutable output, exact single match, checked I/O.
...
int main(int argc, char **argv) {
    ...
    require(argc == 5, "Usage: simple-patch input before after output\n");
    require(!match(argv[1], argv[4]), "simple-patch: output must differ from input\n");
    ...
    patch_file_fd = open(argv[4], O_WRONLY | O_CREAT | O_TRUNC, 0644);
