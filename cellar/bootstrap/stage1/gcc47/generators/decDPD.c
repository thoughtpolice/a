...
/* Cellar: keep all three decimal digits, cover noncanonical DPD encodings,
 * and emit the last partial row. Every table value is checked against GCC. */
uint8_t get_cntrl_char(uint16_t num) {
	if (num == 0) {
		return 0;
	} else if (num < 10) {
		return 1;
...
void bin2char(uint16_t num, uint32_t str[4]) {
...
void bin2bcd8(uint16_t num, char digit[4], uint32_t arr[4]) {
	for (int i = 0; i < 3; i++) {
		arr[i] = digit[i] - '0';
	}
...
int main(void) {
	...
	for (int dpd = 0; dpd < 1024; dpd++) {
		uint32_t bcd = dpd2bcd(dpd);
		char digit[4];
		digit[0] = ((bcd >> 8) & 15) + '0';
		digit[1] = ((bcd >> 4) & 15) + '0';
		digit[2] = (bcd & 15) + '0';
		digit[3] = 0;
		uint32_t bin = (digit[0] - '0') * 100 + (digit[1] - '0') * 10 + digit[2] - '0';
		DPD2BCD[dpd] = bcd;
		DPD2BIN[dpd] = bin;
		DPD2BINK[dpd] = bin * 1000;
		DPD2BINM[dpd] = bin * 1000000;
		bin2bcd8(bin, digit, DPD2BCD8 + (4 * dpd));
	}
	...
	for (int i = 0; i < TABLES_COUNT; i++) {
		...
		for (int j = 0; j < (lengths[i] + 15) / 16; j++) {
