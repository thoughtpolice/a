...
static int emit_text(const char *format, va_list ap) {
    int n=vsnprintf(text+used,sizeof(text)-used,format,ap);
    if(n<0 || n>=sizeof(text)-used) abort(); used+=n; return n;
}
static int emit(void *stream, const char *format, ...) {
    va_list ap; int n;
    (void)stream; va_start(ap,format); n=emit_text(format,ap); va_end(ap); return n;
}
static int emit_styled(void *stream, enum disassembler_style style, const char *format, ...) {
    va_list ap; int n;
    (void)stream; (void)style; va_start(ap,format); n=emit_text(format,ap); va_end(ap); return n;
}
...
int main(void) {
    ...
    init_disassemble_info(&info,NULL,emit,emit_styled);
